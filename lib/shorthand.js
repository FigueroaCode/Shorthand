...
export default {
  ...
  analyzeText(text) {
  //This is for replacing mainClass "name" constructors
    let change = "";
  //if(text.length >= "mainClass".length){
    //  if(text.length > "mainClass".length){
    //    let name = text.substring("mainClass".length + 1, text.length);
    //    change = "class " + name + "\n{\n\tpublic static void main(String args[])\n\t\t{\n//insert code here\n\t\t}\n}";
    //  }else{change = "try that again :("}
  //  }
    //Call appropriate function.

    return mainClassJava;
  }
