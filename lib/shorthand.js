...
export default {
  ...
  analyzeText(text) {
      ...
      //Nested For loops
      length = "nested for".length
      if(newText.substring(0, length) == "nested for"){
          ...
          if( isNaN(end) || isNaN(size)){
            return text;
          }
          let returnVal = this.nestLoops(end, parseInt(size));
          if(returnVal != "fail")
          {
            return returnVal;
          }else{
      ...
      if(newText.substring(0,length) == "mainClass"){
        let nameOfClass = newText.substring(length + 1, newText.length);
        if(nameOfClass == "")
          return text
        else
  ...
  nestLoops(end, numOfLoops){
    if(numOfLoops < 26){//dont more than 27 nested for loops
